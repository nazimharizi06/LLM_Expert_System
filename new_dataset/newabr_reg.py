...
scoring = {
    'a20': make_scorer(Accuracy_score3, greater_is_better=True),
    'mae': make_scorer(mean_absolute_error, greater_is_better=False)  # typical usage is to minimize
}

# Initialize result arrays
rows = len(trees) * len(l_rate)
all_cv_scores_a20 = np.zeros((rows, cv_folds))
all_cv_scores_mae = np.zeros((rows, cv_folds))
current_row = 0

# Loop through learning rates and number of estimators
for rate in l_rate:
    for n_trees in trees:
        abr = AdaBoostRegressor(
            estimator=DecisionTreeRegressor(max_depth=12, random_state=17, criterion='squared_error'),
        ...
        cv_results = cross_validate(abr, X_train, y_train, cv=cv, scoring=scoring)

        all_cv_scores_a20[current_row, :] = cv_results['test_a20']
        all_cv_scores_mae[current_row, :] = cv_results['test_mae']

        print(f'\nTrial #{current_row} | n_estimators={n_trees} | learning_rate={rate}')
        print('a20 mean:', round(cv_results['test_a20'].mean(), 4))
        print('MAE mean:', round(cv_results['test_mae'].mean(), 4))

        current_row += 1

df_metricsA20 = pd.DataFrame(all_cv_scores_a20, columns=[f'Fold {i+1}' for i in range(cv_folds)])
#MAE
df_metrics_MAE = pd.DataFrame(all_cv_scores_mae, columns=[f'Fold {i+1}' for i in range(cv_folds)])
...
with pd.ExcelWriter(file_path, mode='a', engine='openpyxl') as writer:
    ...
    df_metricsA20.to_excel(writer, sheet_name='ABR8_A20_est', index_label='Depth')
    df_metrics_MAE.to_excel(writer, sheet_name='ABR8_MAE_est', index_label='Depth')